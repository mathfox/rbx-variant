...
export interface IsTypeFunc<K extends string> {
	 ...
	 * @returns A user-defined type guard indicating if the instance is of a given type.
	 */
	isType<O extends Record<K, string>, T extends O[K] | VariantCreator<O[K], Func, K>>(
		this: void,
		object: O | undefined,
		type: T,
...
export function isTypeImpl<K extends string>(key: K): IsTypeFunc<K> {
	...
	function isType<T extends O[K] | VariantCreator<O[K], Func, K>, O extends Record<K, string>>(
		...
		if (instanceOrType !== undefined) {
			if (typeIs(instanceOrType, "function") || typeIs(instanceOrType, "string")) {
				const typeArg = instanceOrType as T;
				const typeStr = typeIs(typeArg, "string") ? typeArg : (typeArg as VariantCreator<string, any, K>).output.type;
				...
				const typeStr = typeIs(t, "string") ? t : (t as VariantCreator<string, any, K>).output.type;
