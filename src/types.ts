...
export interface TypesFunc<K extends string> {
	 ...
	 */
	types<T extends VariantModule<K>>(this: void, content: T): Array<Identity<TypesOf<T>>>;
	/**
	 ...
	 * @returns list of string literal types.
	 */
	types<C extends VariantCreator<string, Func, K>>(this: void, content: Array<C>): Array<C["output"]["type"]>;
	/**
...
export function typesImpl<K extends string>(key: K): TypesFunc<K> {
	function types(content: VariantModule<K> | Array<VariantCreator<string, Func, K>> | Array<Record<K, string>>) {
		if (isArray(content)) {
			if (!content.isEmpty() && isVariantCreator(content[0])) {
				return (content as Array<VariantCreator<string, Func, K>>).map((c) => c.output.type);
			} else {
