...
export interface IsOfVariantFunc<K extends string> {
	 ...
	 */
	isOfVariant<T extends VariantModule<K>>(this: void, variant: T): (instance: {} | undefined) => instance is SumType<T>;
}

export function isOfVariantImpl<K extends string>(key: K): IsOfVariantFunc<K> {
	function isOfVariant<T extends VariantModule<K>>(instance: {} | undefined, variant: T): instance is SumType<T>;
	function isOfVariant<T extends VariantModule<K>>(variant: T): (instance: {} | undefined) => instance is SumType<T>;
	function isOfVariant<T extends VariantModule<K>>(...args: [T] | [{} | undefined, T]) {
		if (args.size() === 1) {
			const [variant] = args as [T];
			return (instance: {} | undefined) =>
				instance !== undefined && values(variant).some((vc) => vc.output.type === (instance as Record<K, string>)[key]);
		} else if (args.size() === 2) {
